...
public class AutocropTestChecker {
	...
	public void checkGeneralValues(AutocropResult foundResult) {
		System.out.println("Crop(s) : (target) " + target.getCropNb()
		                 + " / (found) " + foundResult.getCropNb());
	...
	public void checkCoordinates(AutocropResult foundResult){
		...
		System.out.println("Crops found overlapping (at least 80% overlapped) with targeted ones (="+ target.getCropNb()+") = "+ overlappingCrops);
		assertTrue(overlappingCrops >= target.getCropNb() * VALID_CROP_PERCENTAGE/100); // To change: valid if 90% of the crops found
	}
	...
	/**
	 * Calculate the intersection area between two 3D boxes which are aligned/non-rotated
	 * and return the percentage of the area of the crop whose the most overlapped
